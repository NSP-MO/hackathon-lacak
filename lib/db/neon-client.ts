...
export async function createBatch(batch: ProductBatch): Promise<void> {
  console.log("[v0] createBatch repository called for batch:", batch.id)

  await ensureDatabaseReady()

  try {
    console.log("[v0] Inserting product batch...")
    await sql`
    ...
    `
    console.log("[v0] Product batch inserted successfully")

    // Batch insert codes for better performance
    if (batch.codes.length > 0) {
      console.log("[v0] Inserting", batch.codes.length, "product codes...")
      ...
      for (let i = 0; i < batch.codes.length; i++) {
        ...
        console.log(`[v0] Inserting code ${i + 1}/${batch.codes.length}: ${code.id}`)
      ...
      console.log("[v0] All product codes inserted successfully")
    }
  } catch (error) {
    console.error("[v0] Error in createBatch repository:", error)
    throw error
  }
}
